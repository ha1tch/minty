...
func (e *Element) Render(w io.Writer) error {
	...
	// Write attributes in sorted key order for deterministic output --
	// Go's map iteration order is randomized, so without this, the same
	// element can render with different attribute orders across runs
	// (github.com/mogsie, consistent-attribute-order branch).
